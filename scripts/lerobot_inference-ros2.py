...
# Policy imports will be done dynamically based on the policy parameter
...
def get_policy_class(policy_type):
    """
    Dynamically import and return the appropriate policy class based on policy_type
    """
    if policy_type.lower() == 'act':
        ...
        raise ValueError(f"Unsupported policy type: {policy_type}. Supported types: 'act', 'diffusion'")


def make_policy(args):
    """
    Create LeRobot policy from pretrained model based on policy type
    """
    ...
    if dataset_stats is not None:
        try:
            # 先加载默认policy
            policy = PolicyClass.from_pretrained(args.pretrained_model_name_or_path)
    ...
    print(f"{args.policy.upper()} Policy loaded on device: {device}")
...
def get_arguments():
    ...
    parser.add_argument('--policy', type=str, required=False, default='act',
                        help='Policy type to use: act, diffusion, etc.')
